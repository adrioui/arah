...
export const CoverageState = Schema.Literals(["covered", "not-covered", "stale", "unavailable"]);
...
export function parseRequestBody(body: string): RouteRequest | null {
  const exit = Schema.decodeUnknownExit(Schema.fromJsonString(RouteRequest))(body);
